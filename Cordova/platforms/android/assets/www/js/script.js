...
document.addEventListener('deviceready', function() {
  ...
  $('#form-login').submit(function(){
    ...
    // } else {
      // ipServer = 'http://' + $('#ip-server').val() + ':3000';
...
function construireLib(librairie){
  ...
  for(var i=0;i<librairie.length;i++){
    numLib++
    var songIndex = librairie[i].index;
    songLocalImage = librairie[i].image;
    var art = librairie[i].artiste;
    var tit = librairie[i].titre;
    console.log(numLib + ". " + art + " - " + tit);
    $('#librairie ul').append('<li><div class="indexLibrairie">'+ songIndex +'</div><div class="li-text"><img src="'+ songLocalImage +'" alt="" />' + art + " - " + tit +'</div><a href="#" id="btnAjouter-' + numLib + '" class="btn btn-sm btn-success btn-ajouter">Ajouter</a></li>');
  }
...
function construirePlaylist(playlist){
  ...
  for(var i=0;i<playlist.length;i++){
    // console.log(i);
    numPlay++
    var songIndex = playlist[i].index;
    ...
    $('#playlist ul').append('<li><span class="indexPlaylist">'+ songIndex +'</span><div class="li-text"><img src="'+ songLocalImage +'" alt="" />' + art + " - " + tit +'</div><div class="btn-group btn-group-vote" role="group" aria-label="Basic example"><button id="btn-show-vote" type="button" class="btn btn-sm disabled">'+ vote +'</button><button type="button" id="btnVote-' + numPlay + '" class="btn btn-sm btn-success btn-voter"><i class="fa fa-thumbs-up" aria-hidden="true"></i></button></div></li>');
    if(i<currentLocalSong+1){
      $('#btnVote-'+i).text("Joué");
      $('#btnVote-'+i).addClass('btn-played');
      $('#btnVote-'+i).removeClass('btn-success');
      $('#btnVote-'+i).prop('disabled', true);
      $('#btnVote-'+i).parent().removeClass('btn-group');
      $('#btnVote-'+i).prev().hide();
